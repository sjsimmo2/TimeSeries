...
consume_modelt2<-as.data.frame(consume_modelt1)
t(consume_modelt2)

glance(consume_modelt1) %>% arrange(AICc) %>% select(.model:BIC)
...
hurricane = pd.read_csv("https://raw.githubusercontent.com/sjsimmo2/TimeSeries/master/hurrican.csv")
hurricane2 = hurricane.dropna(axis=0)
hurricane_train =hurricane2.head(149)
hurricane_train
...
model_Quotes = StatsForecast(models = [ARIMA(order=(1, 0, 0))], freq = 'ME')
...
model_Quotes2 = StatsForecast(models = [ARIMA(order=(0, 0, 1))], freq = 'ME')
