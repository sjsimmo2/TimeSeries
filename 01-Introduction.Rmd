...
For most of the applications we will be using, you will first need to create a tsibble.  In order to create a tsibble, you need to have a column that identifies the date (which we will indicate with "index").  The code below creates tsibble for the Steel data set and the Minneapolis temperature data set:
...
Another example of creating a tsibble with the US Airlines data set:
...
Some other interesting plots we can look at is the seasonal plot (only if our time series has a seasonal component....if you have yearly data, this is not possible since each line represents one year). 
...
We can visualize the time series with the trend component overlaid with it: 
...
We can also do a classical decomposition (this can do an additive decomposition and a multiplicative decomposition).
...
```
We can also look at the strength of the trend and seasonal components in a decomposition (within the STL decomposition):
